...
docker exec dspacedb createdb -U postgres -O dspace --encoding=UNICODE dspace

echo "$(tput setaf 6)Creating pgcrypto extension...$(tput sgr 0)"
docker exec dspacedb psql -U postgres dspace -c 'CREATE EXTENSION pgcrypto;'

echo "$(tput setaf 6)Creating dspace user...$(tput sgr 0)"
docker exec dspacedb psql -U postgres dspace -c 'ALTER USER dspace superuser;'
...
echo "$(tput setaf 6)Vacuum database...$(tput sgr 0)"
docker exec dspacedb vacuumdb -U postgres dspace
...
docker exec dspacedb psql -U postgres dspace -c 'ALTER USER dspace nosuperuser;'
